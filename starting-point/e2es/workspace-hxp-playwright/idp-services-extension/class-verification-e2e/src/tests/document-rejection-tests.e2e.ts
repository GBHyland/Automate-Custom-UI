...
test.describe('Workspace IDP Tests - Start IDP process', () => {
    ...
    test(`[XAT-17694] Reject Document Testing`, async ({
        ...
        // Reject button should not be enabled when only rejected documents are selected
        ...
        await expect(idpClassificationPage.floaterToolbar.rejectButton).toHaveCount(0);
    });
});
